...
public class DiaPersonController {
     ...
     */
    @RequestMapping("/my")
    public ModelAndView my( HttpServletRequest request,HttpSession session)throws Exception{
        ...
        if(cookies!=null){
            for(Cookie cookie:cookies){
                if(cookie.getName().equals("recorder")){
                    ...
                    mav.addObject("viewCount", viewGoodslist.size());
                    break;
                }
            }
        }
